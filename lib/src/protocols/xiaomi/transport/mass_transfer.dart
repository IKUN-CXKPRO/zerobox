import 'dart:async';
import 'dart:collection';
import 'dart:typed_data';

import 'package:crypto/crypto.dart' as crypto;

import 'package:zerobox/src/core/logging/logging_service.dart';
import 'package:zerobox/src/protocols/common/device_protocol.dart';
import 'package:zerobox/src/protocols/generated/xiaomi/wear.pb.dart' as pb;
import 'package:zerobox/src/protocols/generated/xiaomi/wear_common.pbenum.dart'
    as pb_common;
import 'package:zerobox/src/protocols/generated/xiaomi/wear_mass.pb.dart';
import 'package:zerobox/src/protocols/xiaomi/commands/xiaomi_request_pool.dart';
import 'package:zerobox/src/protocols/xiaomi/packet/l2_packet.dart';
import 'package:zerobox/src/protocols/xiaomi/packet/mass_packet.dart';
import 'package:zerobox/src/protocols/xiaomi/transport/xiaomi_sar_controller.dart';

class MassConfig {
  const MassConfig({
    this.ackWaitTimeoutSecs = 30,
    this.ackPollIntervalMs = 50,
    this.ackStallDefaultMs = 400,
    this.ackStallMinMs = 120,
    this.ackStallMaxMs = 900,
    this.backlogMultiplier = 6,
    this.maxBatchParts = 32,
    this.fallbackBatchParts = 8,
    this.fallbackBacklogLimit = 96,
  });

  final int ackWaitTimeoutSecs;
  final int ackPollIntervalMs;
  final int ackStallDefaultMs;
  final int ackStallMinMs;
  final int ackStallMaxMs;
  final int backlogMultiplier;
  final int maxBatchParts;
  final int fallbackBatchParts;
  final int fallbackBacklogLimit;
}

class SendMassCallbackData {
  SendMassCallbackData({
    required this.progress,
    required this.totalParts,
    required this.currentPartNum,
    required this.actualDataPayloadLen,
  });

  final double progress;
  final int totalParts;
  final int currentPartNum;
  final int actualDataPayloadLen;
}

class ReceiveMassCallbackData {
  ReceiveMassCallbackData({
    required this.channel,
    required this.progress,
    required this.totalParts,
    required this.currentPartNum,
    required this.fileName,
  });

  final int channel;
  final double progress;
  final int totalParts;
  final int currentPartNum;
  final String fileName;
}

class ReverseMassReceiveResult {
  ReverseMassReceiveResult({
    required this.channel,
    required this.fileName,
    required this.data,
  });

  final L2Channel channel;
  final String fileName;
  final Uint8List data;
}

class _PendingMassPart {
  _PendingMassPart({
    required this.partNum,
    required this.seq,
    required this.payloadLen,
  });

  final int partNum;
  final int seq;
  final int payloadLen;
  bool acked = false;
}

class ReverseMassWaiter {
  ReverseMassWaiter({
    required this.packet,
    required this.progressCb,
    required this.completer,
    required this.siblings,
  });

  final ReverseMassPacket packet;
  final void Function(ReceiveMassCallbackData) progressCb;
  final Completer<ReverseMassReceiveResult> completer;
  final List<int> siblings;
}

class ReverseMassPacket {
  ReverseMassPacket()
      : file = {},
        fileName = '',
        header = Uint8List(0),
        totalPart = 0,
        curPart = 0,
        error = false,
        empty = true;

  String fileName;
  int totalPart;
  int curPart;
  Uint8List header;
  final Map<int, Uint8List> file;
  bool error;
  bool empty;

  void handlePacket(Uint8List packet) {
    empty = false;

    if (packet.length < 12) {
      error = true;
      throw ProtocolException('Invalid reverse mass packet');
    }
    if (packet[0] != 0) {
      error = true;
      throw ProtocolException('Invalid packet version');
    }

    final total = packet[2] | (packet[3] << 8);
    final cur = packet[4] | (packet[5] << 8);

    late final int skipOffset;
    if (totalPart == 0) {
      final len = packet[6];
      if (7 + len + 5 > packet.length) {
        error = true;
        throw ProtocolException('Invalid reverse mass filename length');
      }
      fileName = String.fromCharCodes(packet.sublist(7, 7 + len));
      header = Uint8List.sublistView(packet, 6, 7 + len + 5);
      skipOffset = 7 + len + 5;
    } else {
      if (total != totalPart) {
        error = true;
        throw ProtocolException(
          'Invalid total $total != $totalPart',
        );
      }
      skipOffset = 6;
    }

    if (cur == total) {
      if (packet.length < skipOffset + 4) {
        error = true;
        throw ProtocolException('Invalid reverse mass final packet');
      }
      file[cur] = Uint8List.sublistView(
        packet,
        skipOffset,
        packet.length - 4,
      );

      final checkData = BytesBuilder()
        ..add(header)
        ..add(assembleFile());

      final crc32 = packet[packet.length - 4] |
          (packet[packet.length - 3] << 8) |
          (packet[packet.length - 2] << 16) |
          (packet[packet.length - 1] << 24);
      final dataCrc32 = _crc32(Uint8List.fromList(checkData.toBytes()));

      if (crc32 != dataCrc32) {
        error = true;
        throw ProtocolException('Invalid crc32 $crc32 != $dataCrc32');
      }
    } else {
      file[cur] = Uint8List.sublistView(packet, skipOffset);
    }

    totalPart = total;
    curPart += 1;
  }

  bool get complete => curPart == totalPart && totalPart != 0;

  Uint8List assembleFile() {
    final result = BytesBuilder();
    for (var key = 1; key <= totalPart; key++) {
      final vec = file[key];
      if (vec == null) {
        throw ProtocolException('block $key was not found!');
      }
      result.add(vec);
    }
    return result.toBytes();
  }

  static bool looksLikeReverseMassPacket(Uint8List payload) {
    return payload.length >= 12 && payload[0] == 0;
  }
}

class MassTransfer {
  MassTransfer({
    required this.sendPbPacket,
    required this.sar,
    required this.requestPool,
    required this.deviceAddr,
    this.config = const MassConfig(),
  }) : _log = getLogger('MassTransfer');

  final Future<void> Function(pb.WearPacket packet) sendPbPacket;
  final XiaomiSarController sar;
  final XiaomiRequestPool requestPool;
  final String deviceAddr;
  final MassConfig config;
  final Logger _log;

  final _reverseMassWaits = <int, ReverseMassWaiter>{};
  Map<int, ReverseMassWaiter> get reverseMassWaits => _reverseMassWaits;

  Future<void> sendFile({
    required Uint8List fileData,
    required MassDataType dataType,
    void Function(SendMassCallbackData data)? onProgress,
  }) async {
    return sendFileWithKnownSliceLength(
      fileData: fileData,
      dataType: dataType,
      onProgress: onProgress,
    );
  }

  Future<void> sendFileWithKnownSliceLength({
    required Uint8List fileData,
    required MassDataType dataType,
    int? expectedSliceLength,
    void Function(SendMassCallbackData data)? onProgress,
  }) async {
    final fileMd5 = _md5(fileData);
    final fileLen = fileData.length;

    _log.info('Building MASS Prepare response listener...');

    final prepareResponse = await requestPool.request<PrepareResponse>(
      packet: _buildMassPrepareRequest(dataType, fileMd5, fileLen),
      typeMatcher: (p) =>
          p.whichPayload() == pb.WearPacket_Payload.mass &&
          p.id == Mass_MassID.PREPARE.value,
      responseMapper: (p) => p.mass.prepareResponse,
      timeout: const Duration(seconds: 10),
    );

    if (prepareResponse.prepareStatus != pb_common.PrepareStatus.READY) {
      throw ProtocolException(
        'Mass data prepare was not READY. '
        'Please check whether the device has enough storage space and try again.',
      );
    }

    final sliceLength = expectedSliceLength ??
        (prepareResponse.hasExpectedSliceLength()
            ? prepareResponse.expectedSliceLength
            : 244);

    final remainedDataLength = prepareResponse.hasRemainedDataLength()
        ? prepareResponse.remainedDataLength
        : 0;
    var sentLength = remainedDataLength.clamp(0, fileLen);
    if (sentLength > 0) {
      _log.info(
        '[Mass] device retained $sentLength / $fileLen bytes from a previous transfer, resuming from there',
      );
    }

    return _sendFileWithSliceLength(
      fileData: fileData,
      dataType: dataType,
      expectedSliceLength: sliceLength,
      sentLength: sentLength,
      onProgress: onProgress,
    );
  }

  Future<ReverseMassReceiveResult> beginReverseMassReceive(
    L2Channel channel, {
    required void Function(ReceiveMassCallbackData) progressCb,
  }) {
    return beginReverseMassReceiveMulti([channel], progressCb: progressCb);
  }

  Future<ReverseMassReceiveResult> beginReverseMassReceiveMulti(
    List<L2Channel> channels, {
    required void Function(ReceiveMassCallbackData) progressCb,
  }) {
    if (channels.isEmpty) {
      throw ArgumentError('reverse MASS receive requires at least one channel');
    }
    for (final channel in channels) {
      final key = channel.value;
      if (_reverseMassWaits.containsKey(key)) {
        throw StateError(
          'reverse MASS receive already in progress on channel $channel',
        );
      }
    }

    final completer = Completer<ReverseMassReceiveResult>();
    final siblings = channels.map((c) => c.value).toList();

    for (final channel in channels) {
      final key = channel.value;
      final otherSiblings = siblings.where((s) => s != key).toList();
      _reverseMassWaits[key] = ReverseMassWaiter(
        packet: ReverseMassPacket(),
        progressCb: progressCb,
        completer: completer,
        siblings: otherSiblings,
      );
    }

    return completer.future;
  }

  void clearReverseMassWait(L2Channel channel) {
    final key = channel.value;
    final waiter = _reverseMassWaits[key];
    final siblings = waiter?.siblings ?? [];
    _reverseMassWaits.remove(key);
    for (final sibling in siblings) {
      _reverseMassWaits.remove(sibling);
    }
  }

  void handleReverseMassPayload(L2Channel channel, Uint8List payload) {
    final key = channel.value;
    final waiter = _reverseMassWaits[key];
    if (waiter == null) return;

    if (waiter.packet.empty &&
        !ReverseMassPacket.looksLikeReverseMassPacket(payload)) {
      return;
    }

    void Function(ReceiveMassCallbackData)? progressCb;
    ReceiveMassCallbackData? progress;
    ReverseMassReceiveResult? completion;
    var shouldRemove = false;

    try {
      waiter.packet.handlePacket(payload);
      progressCb = waiter.progressCb;
      final totalParts = waiter.packet.totalPart;
      final currentPartNum = waiter.packet.curPart;
      final fileName = waiter.packet.fileName;
      progress = ReceiveMassCallbackData(
        channel: key,
        progress: totalParts == 0 ? 0.0 : currentPartNum / totalParts,
        totalParts: totalParts,
        currentPartNum: currentPartNum,
        fileName: fileName,
      );

      if (waiter.packet.complete) {
        shouldRemove = true;
        completion = ReverseMassReceiveResult(
          channel: channel,
          fileName: fileName,
          data: waiter.packet.assembleFile(),
        );
      }
    } catch (err) {
      shouldRemove = true;
      waiter.completer.completeError(err);
    }

    if (progressCb != null && progress != null) {
      progressCb(progress);
    }

    if (!shouldRemove) return;

    final removed = _reverseMassWaits.remove(key);
    if (removed == null) return;
    for (final sibling in removed.siblings) {
      _reverseMassWaits.remove(sibling);
    }

    if (completion != null && !removed.completer.isCompleted) {
      removed.completer.complete(completion);
    }
  }

  Future<void> _sendFileWithSliceLength({
    required Uint8List fileData,
    required MassDataType dataType,
    required int expectedSliceLength,
    required int sentLength,
    void Function(SendMassCallbackData data)? onProgress,
  }) async {
    final fileLen = fileData.length;
    final effectiveSentLength = sentLength.clamp(0, fileLen);
    final progressBase = fileLen == 0
        ? 0.0
        : (effectiveSentLength / fileLen).clamp(0.0, 1.0);

    final massPacket = MassPacket.build(fileData, dataType);
    final massInnerPayloadWithCrc32 = massPacket.encodeWithCrc32(
      sentLength: effectiveSentLength,
    );

    final massFragmentMaxLen = expectedSliceLength - (1 + 1 + 2 + 2);
    if (massFragmentMaxLen <= 0) {
      throw ProtocolException(
        'Calculated mass_fragment_max_len is 0. Device limit ($expectedSliceLength) is too small.',
      );
    }

    final totalParts =
        ((massInnerPayloadWithCrc32.length / massFragmentMaxLen).ceil())
            .clamp(0, 65535);
    if (totalParts == 0 && massInnerPayloadWithCrc32.isNotEmpty) {
      throw ProtocolException('Calculated total_parts is 0 for non-empty payload.');
    }

    final sendTimeoutHintMs = sar.sendTimeoutMs;

    // 经验值：一次不要发太多，4 个一批比较稳；backlog 也不要太大。
    const batchLimit = 4;
    const backlogSoftLimit = 12;
    final ackStallDeadline = _computeAckStallDeadline(8, sendTimeoutHintMs);

    _log.info(
      '[Mass] send setup: total_parts=$totalParts, '
      'fragment_max_len=$massFragmentMaxLen, batch_limit=$batchLimit, backlog_limit=$backlogSoftLimit',
    );

    final pendingParts = Queue<_PendingMassPart>();
    final batchPayloads = <Uint8List>[];
    final batchMeta = <(int, int)>[];
    var lastProgressAt = DateTime.now();
    var flushCount = 0;

    for (var i = 0; i < totalParts; i++) {
      final currentPartNum = i + 1;
      final startIndex = i * massFragmentMaxLen;
      final endIndex = (startIndex + massFragmentMaxLen < massInnerPayloadWithCrc32.length)
          ? startIndex + massFragmentMaxLen
          : massInnerPayloadWithCrc32.length;
      final fragment = Uint8List.sublistView(
        massInnerPayloadWithCrc32,
        startIndex,
        endIndex,
      );

      final actualDataPayload = BytesBuilder();
      _writeUint16LE(actualDataPayload, totalParts);
      _writeUint16LE(actualDataPayload, currentPartNum);
      actualDataPayload.add(fragment);

      final actualDataPayloadLen = actualDataPayload.length;
      final l2 = L2Packet(
        channel: L2Channel.mass,
        opcode: L2OpCode.write,
        payload: actualDataPayload.toBytes(),
      );
      batchPayloads.add(l2.toBytes());
      batchMeta.add((currentPartNum, actualDataPayloadLen));

      onProgress?.call(SendMassCallbackData(
        progress: progressBase + (1.0 - progressBase) * (currentPartNum / totalParts),
        totalParts: totalParts,
        currentPartNum: currentPartNum,
        actualDataPayloadLen: actualDataPayloadLen,
      ));

      if (batchPayloads.length >= batchLimit) {
        flushCount += 1;
        await _flushMassBatch(
          batchPayloads: batchPayloads,
          batchMeta: batchMeta,
          pendingParts: pendingParts,
          flushRound: flushCount,
        );
        await _enforceFlowControl(
          pendingParts: pendingParts,
          totalParts: totalParts,
          progressBase: progressBase,
          onProgress: onProgress,
          ackStallDeadline: ackStallDeadline,
          backlogSoftLimit: backlogSoftLimit,
          lastProgressAt: lastProgressAt,
        );
        lastProgressAt = DateTime.now();
      }
    }

    if (batchPayloads.isNotEmpty) {
      flushCount += 1;
      await _flushMassBatch(
        batchPayloads: batchPayloads,
        batchMeta: batchMeta,
        pendingParts: pendingParts,
        flushRound: flushCount,
      );
    }

    await _enforceFlowControl(
      pendingParts: pendingParts,
      totalParts: totalParts,
      progressBase: progressBase,
      onProgress: onProgress,
      ackStallDeadline: ackStallDeadline,
      backlogSoftLimit: backlogSoftLimit,
      lastProgressAt: lastProgressAt,
    );

    // 已把所有 part 入队；剩余的 ACK 由 SAR 层异步处理/重传，
    // 不再在这里阻塞等待，避免安装结果超时。
    _log.info('[Mass] All mass data parts queued; returning without waiting for ACKs.');
  }

  Future<void> _flushMassBatch({
    required List<Uint8List> batchPayloads,
    required List<(int, int)> batchMeta,
    required Queue<_PendingMassPart> pendingParts,
    required int flushRound,
  }) async {
    if (batchPayloads.isEmpty) return;

    final payloads = List<Uint8List>.from(batchPayloads);
    final meta = List<(int, int)>.from(batchMeta);
    batchPayloads.clear();
    batchMeta.clear();

    final seqs = sar.enqueueBatch(payloads);
    if (seqs.length != meta.length) {
      throw ProtocolException(
        'enqueue_batch returned ${seqs.length} seqs but ${meta.length} payloads were submitted',
      );
    }

    for (var i = 0; i < meta.length; i++) {
      final (partNum, payloadLen) = meta[i];
      pendingParts.add(_PendingMassPart(
        partNum: partNum,
        seq: seqs[i],
        payloadLen: payloadLen,
      ));
    }

    _log.fine(
      '[Mass] flush_batch round=$flushRound payloads=${meta.length}',
    );
  }

  Future<void> _enforceFlowControl({
    required Queue<_PendingMassPart> pendingParts,
    required int totalParts,
    required double progressBase,
    required void Function(SendMassCallbackData)? onProgress,
    required Duration ackStallDeadline,
    required int backlogSoftLimit,
    required DateTime lastProgressAt,
  }) async {
    final consumed = await _consumeAckedParts(
      pendingParts: pendingParts,
      totalParts: totalParts,
      progressBase: progressBase,
      onProgress: onProgress,
    );
    final mutableLastProgressAt = consumed > 0 ? DateTime.now() : lastProgressAt;

    if (pendingParts.isEmpty) return;

    final now = DateTime.now();
    var shouldWait = pendingParts.length >= backlogSoftLimit;
    if (!shouldWait && now.difference(mutableLastProgressAt) >= ackStallDeadline) {
      shouldWait = true;
    }

    if (shouldWait) {
      final frontSeq = pendingParts.first.seq;
      final waitReason = pendingParts.length >= backlogSoftLimit ? 'backlog' : 'stall';
      _log.fine('[Mass] flow_wait reason=$waitReason pending=${pendingParts.length}');
      await _waitForSeqAck(frontSeq);
      await _consumeAckedParts(
        pendingParts: pendingParts,
        totalParts: totalParts,
        progressBase: progressBase,
        onProgress: onProgress,
      );
    }
  }

  Future<void> _waitForSeqAck(int seq) async {
    final startedAt = DateTime.now();
    final timeout = Duration(seconds: config.ackWaitTimeoutSecs);

    while (!sar.isAcked(seq)) {
      final elapsed = DateTime.now().difference(startedAt);
      if (elapsed >= timeout) {
        throw ProtocolException('Timeout waiting for mass packet ACK seq=$seq');
      }

      final remaining = timeout - elapsed;
      try {
        await sar.ackNotifier.first.timeout(remaining);
      } on TimeoutException catch (_) {
        throw ProtocolException('Timeout waiting for mass packet ACK seq=$seq');
      }
    }
  }

  Future<int> _consumeAckedParts({
    required Queue<_PendingMassPart> pendingParts,
    required int totalParts,
    required double progressBase,
    required void Function(SendMassCallbackData)? onProgress,
  }) async {
    var consumed = 0;
    while (pendingParts.isNotEmpty) {
      final front = pendingParts.first;
      if (!front.acked) {
        if (!sar.isAcked(front.seq)) break;
        front.acked = true;
        sar.markAckConsumed(front.seq);
      }
      pendingParts.removeFirst();
      consumed++;
    }
    return consumed;
  }

  Duration _computeAckStallDeadline(int? windowHint, int sendTimeoutMs) {
    final fromWindow = windowHint != null
        ? windowHint * config.ackPollIntervalMs * 3
        : config.ackStallDefaultMs;

    final timeoutCandidate = sendTimeoutMs > 0 && sendTimeoutMs < 0xFFFFFFFF
        ? (sendTimeoutMs ~/ 8).max(config.ackStallMinMs)
        : fromWindow;

    final combined = fromWindow.min(timeoutCandidate);
    final clamped = combined.clamp(config.ackStallMinMs, config.ackStallMaxMs);
    return Duration(milliseconds: clamped);
  }

  pb.WearPacket _buildMassPrepareRequest(
    MassDataType dataType,
    Uint8List fileMd5,
    int fileLength,
  ) {
    final massPayload = PrepareRequest(
      dataType: dataType.value,
      dataId: fileMd5,
      dataLength: fileLength,
    );

    final massPkt = Mass(prepareRequest: massPayload);

    return pb.WearPacket(
      type: pb.WearPacket_Type.MASS,
      id: Mass_MassID.PREPARE.value,
      mass: massPkt,
    );
  }
}

Uint8List _md5(Uint8List data) {
  return Uint8List.fromList(crypto.md5.convert(data).bytes);
}

void _writeUint16LE(BytesBuilder builder, int value) {
  builder.addByte(value & 0xFF);
  builder.addByte((value >> 8) & 0xFF);
}

int _crc32(Uint8List data) {
  const table = [
    0x00000000, 0x77073096, 0xee0e612c, 0x990951ba,
    0x076dc419, 0x706af48f, 0xe963a535, 0x9e6495a3,
    0x0edb8832, 0x79dcb8a4, 0xe0d5e91e, 0x97d2d988,
    0x09b64c2b, 0x7eb17cbd, 0xe7b82d07, 0x90bf1d91,
    0x1db71064, 0x6ab020f2, 0xf3b97148, 0x84be41de,
    0x1adad47d, 0x6ddde4eb, 0xf4d4b551, 0x83d385c7,
    0x136c9856, 0x646ba8c0, 0xfd62f97a, 0x8a65c9ec,
    0x14015c4f, 0x63066cd9, 0xfa0f3d63, 0x8d080df5,
    0x3b6e20c8, 0x4c69105e, 0xd56041e4, 0xa2677172,
    0x3c03e4d1, 0x4b04d447, 0xd20d85fd, 0xa50ab56b,
    0x35b5a8fa, 0x42b2986c, 0xdbbbc9d6, 0xacbcf940,
    0x32d86ce3, 0x45df5c75, 0xdcd60dcf, 0xabd13d59,
    0x26d930ac, 0x51de003a, 0xc8d75180, 0xbfd06116,
    0x21b4f4b5, 0x56b3c423, 0xcfba9599, 0xb8bda50f,
    0x2802b89e, 0x5f058808, 0xc60cd9b2, 0xb10be924,
    0x2f6f7c87, 0x58684c11, 0xc1611dab, 0xb6662d3d,
    0x76dc4190, 0x01db7106, 0x98d220bc, 0xefd5102a,
    0x71b18589, 0x06b6b51f, 0x9fbfe4a5, 0xe8b8d433,
    0x7807c9a2, 0x0f00f934, 0x9609a88e, 0xe10e9818,
    0x7f6a0dbb, 0x086d3d2d, 0x91646c97, 0xe6635c01,
    0x6b6b51f4, 0x1c6c6162, 0x856530d8, 0xf262004e,
    0x6c0695ed, 0x1b01a57b, 0x8208f4c1, 0xf50fc457,
    0x65b0d9c6, 0x12b7e950, 0x8bbeb8ea, 0xfcb9887c,
    0x62dd1ddf, 0x15da2d49, 0x8cd37cf3, 0xfbd44c65,
    0x4db26158, 0x3ab551ce, 0xa3bc0074, 0xd4bb30e2,
    0x4adfa541, 0x3dd895d7, 0xa4d1c46d, 0xd3d6f4fb,
    0x4369e96a, 0x346ed9fc, 0xad678846, 0xda60b8d0,
    0x44042d73, 0x33031de5, 0xaa0a4c5f, 0xdd0d7cc9,
    0x5005713c, 0x270241aa, 0xbe0b1010, 0xc90c2086,
    0x5768b525, 0x206f85b3, 0xb966d409, 0xce61e49f,
    0x5edef90e, 0x29d9c998, 0xb0d09822, 0xc7d7a8b4,
    0x59b33d17, 0x2eb40d81, 0xb7bd5c3b, 0xc0ba6cad,
    0xedb88320, 0x9abfb3b6, 0x03b6e20c, 0x74b1d29a,
    0xead54739, 0x9dd277af, 0x04db2615, 0x73dc1683,
    0xe3630b12, 0x94643b84, 0x0d6d6a3e, 0x7a6a5aa8,
    0xe40ecf0b, 0x9309ff9d, 0x0a00ae27, 0x7d079eb1,
    0xf00f9344, 0x8708a3d2, 0x1e01f268, 0x6906c2fe,
    0xf762575d, 0x806567cb, 0x196c3671, 0x6e6b06e7,
    0xfed41b76, 0x89d32be0, 0x10da7a5a, 0x67dd4acc,
    0xf9b9df6f, 0x8ebeeff9, 0x17b7be43, 0x60b08ed5,
    0xd6d6a3e8, 0xa1d1937e, 0x38d8c2c4, 0x4fdff252,
    0xd1bb67f1, 0xa6bc5767, 0x3fb506dd, 0x48b2364b,
    0xd80d2bda, 0xaf0a1b4c, 0x36034af6, 0x41047a60,
    0xdf60efc3, 0xa867df55, 0x316e8eef, 0x4669be79,
    0xcb61b38c, 0xbc66831a, 0x256fd2a0, 0x5268e236,
    0xcc0c7795, 0xbb0b4703, 0x220216b9, 0x5505262f,
    0xc5ba3bbe, 0xb2bd0b28, 0x2bb45a92, 0x5cb36a04,
    0xc2d7ffa7, 0xb5d0cf31, 0x2cd99e8b, 0x5bdeae1d,
    0x9b64c2b0, 0xec63f226, 0x756aa39c, 0x026d930a,
    0x9c0906a9, 0xeb0e363f, 0x72076785, 0x05005713,
    0x95bf4a82, 0xe2b87a14, 0x7bb12bae, 0x0cb61b38,
    0x92d28e9b, 0xe5d5be0d, 0x7cdcefb7, 0x0bdbdf21,
    0x86d3d2d4, 0xf1d4e242, 0x68ddb3f8, 0x1fda836e,
    0x81be16cd, 0xf6b9265b, 0x6fb077e1, 0x18b74777,
    0x88085ae6, 0xff0f6a70, 0x66063bca, 0x11010b5c,
    0x8f659eff, 0xf862ae69, 0x616bffd3, 0x166ccf45,
    0xa00ae278, 0xd70dd2ee, 0x4e048354, 0x3903b3c2,
    0xa7672661, 0xd06016f7, 0x4969474d, 0x3e6e77db,
    0xaed16a4a, 0xd9d65adc, 0x40df0b66, 0x37d83bf0,
    0xa9bcae53, 0xdebb9ec5, 0x47b2cf7f, 0x30b5ffe9,
    0xbdbdf21c, 0xcabac28a, 0x53b39330, 0x24b4a3a6,
    0xbad03605, 0xcdd70693, 0x54de5729, 0x23d967bf,
    0xb3667a2e, 0xc4614ab8, 0x5d681b02, 0x2a6f2b94,
    0xb40bbe37, 0xc30c8ea1, 0x5a05df1b, 0x2d02ef8d,
  ];

  var crc = 0xFFFFFFFF;
  for (final byte in data) {
    crc = (crc >> 8) ^ table[(crc ^ byte) & 0xFF];
  }
  return (crc ^ 0xFFFFFFFF) & 0xFFFFFFFF;
}

extension _IntHelpers on int {
  int max(int other) => this > other ? this : other;
  int min(int other) => this < other ? this : other;
}
